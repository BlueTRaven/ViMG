...
namespace ViMG
{
	public class ObjRegistry<T> where T : class, IRegisterable
	{
		...
		public virtual void AddFromOther(ObjRegistry<T>? other)
		{
			if (other != null)
			{
				// Don't do AddRange - we're relying on override behavior in Register for some things, particularly setting ids,
				// which will need to be re-set
				foreach (T o in other.GetIterable())
				{
					Register(o);
				}
			}
