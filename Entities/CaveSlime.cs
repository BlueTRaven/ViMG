...
{
    public class CaveSlime : Entity, IHitboxOwner
    {
		...
		public override void Update(double deltaTime)
		{
			...
			if (hitbox == -1)
				hitbox = world.HitboxManager.Add(this, Bounds, Vector3.Zero, HitboxManager.Group.ENEMYHOSTILE_BOTH, 4, 1f, invulnTimer <= 0);
		...
		public override void Draw(GraphicsDevice device, Effect effect)
		{
			...
			Main.Renderer.DrawsPassGBuffer.Add(new Rendering.RendererDeferred.GBufferDraw(Main.assetsManager.GetAsset<Texture2D>("slime"),
				DrawHelper.BlackPixel, Main.assetsManager.GetAsset<Texture2D>("slime_emissive"), mesh.VBO, mesh.IBO,
				Matrix.CreateRotationX(Math.Clamp(-Main.camera.Rotation.X, MathHelper.ToRadians(-15), MathHelper.ToRadians(15))) *
				Matrix.CreateRotationY(-Main.camera.Rotation.Y) *
				Matrix.CreateTranslation(Position),
				noticeHandler.Noticed ? new RectangleF(48, ysrc, 16, 16) : new RectangleF(32, ysrc, 16, 16)));
