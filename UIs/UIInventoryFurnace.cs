...
namespace ViMG.UIs
{
	public class UIInventoryFurnace : UIInventory
	{
		...
		public override void Update()
		{
			...
			if ((output = UIInventoryHelper.HandleItemSlot(player, furnaceInventory, 0, itemSlotA, ref held, new UIInventoryHelper.WhiteListNone())) != UIInventoryHelper.ItemSlotClickOutput.None);
			{
				if (output == UIInventoryHelper.ItemSlotClickOutput.NeedsSwapInventory)
					UIInventoryHelper.SwapInventory(furnaceInventory, playerInventory, 0);

				furnaceInventoryUpdated = true;
			}

			if ((output = UIInventoryHelper.HandleItemSlot(player, furnaceInventory, 1, itemSlotB, ref held, new UIInventoryHelper.WhiteListNone())) != UIInventoryHelper.ItemSlotClickOutput.None)
			{
				if (output == UIInventoryHelper.ItemSlotClickOutput.NeedsSwapInventory)
					UIInventoryHelper.SwapInventory(furnaceInventory, playerInventory, 1);

				furnaceInventoryUpdated = true;
			}

			if ((output = UIInventoryHelper.HandleItemSlot(player, furnaceInventory, 2, itemSlotFuel, ref held, new UIInventoryHelper.WhiteListOneName("glowdust"))) != UIInventoryHelper.ItemSlotClickOutput.None)
			{
				if (output == UIInventoryHelper.ItemSlotClickOutput.NeedsSwapInventory)
					UIInventoryHelper.SwapInventory(furnaceInventory, playerInventory, 2);
			...
			{
				player.OpenUI(new UIRecipeBook(Main.Registry.CubeRegistry.Get("furnace_t1") as CubeFurnace, new ItemInstance()));
			}
